...
const rootPackageJson = JSON.parse(readFileSync(join(repoRoot, "package.json"), "utf8")) as { scripts?: Record<string, string>; engines?: Record<string, string> };
...
assert.equal(rootPackageJson.engines?.node, ">=20.19", "workspace Node engine must satisfy Vite 7 requirements.");
...
assert.doesNotMatch(builderConfig, /(?:^|\n)\s*- preload\.cjs/, "legacy task-window preload must not be packaged.");
...
assert.ok(!existsSync(join(appDir, "preload.cjs")), "legacy task-window preload.cjs must be removed from packaging.");
assert.ok(!existsSync(join(appDir, "src", "windows.ts")), "legacy task-window windows.ts must be removed.");
assert.ok(existsSync(join(appDir, "control-center-preload.cjs")), "control-center-preload.cjs must exist for Control Center packaging.");
...
const internalUiProtocolSource = readFileSync(join(appDir, "src", "internal-ui-protocol.ts"), "utf8");
...
const controlCenterWindowSource = readFileSync(join(appDir, "src", "control-center-window.ts"), "utf8");
const controlCenterIpcSource = readFileSync(join(appDir, "src", "control-center-ipc.ts"), "utf8");
const controlCenterIpcCoreSource = readFileSync(join(appDir, "src", "control-center-ipc-core.ts"), "utf8");
const controlCenterContractSource = readFileSync(join(appDir, "src", "shared", "control-center-contract.ts"), "utf8");
const controlCenterRendererSource = readFileSync(join(appDir, "src", "renderer", "control-center", "src", "main.tsx"), "utf8");
const controlCenterRendererHtml = readFileSync(join(appDir, "src", "renderer", "control-center", "index.html"), "utf8");
...
assert.match(mainSource, /installControlCenterIpcHandlers\(\)/, "desktop startup must install Control Center IPC handlers.");
assert.match(traySource, /Open Control Center\.\.\./, "desktop tray must expose the Control Center.");
assert.match(traySource, /Integrations\.\.\.[\s\S]*openControlCenterIntegrationsWindow\(\)/, "desktop tray Integrations must open narrow Control Center Integrations.");
assert.doesNotMatch(traySource, /Integrations\.\.\.[\s\S]*openTaskWindow\("agent-setup"\)/, "desktop tray Integrations must not open the legacy agent-setup task window.");
assert.match(controlCenterPreloadSource, /contextBridge\.exposeInMainWorld\("openPetsControlCenter", api\)/, "Control Center preload must expose only the narrow named API.");
assert.match(controlCenterPreloadSource, /getDashboardSnapshot: \(\) => ipcRenderer\.invoke\("openpets:control-center:get-dashboard-snapshot"\)/, "Control Center preload must expose the dashboard snapshot query.");
for (const method of ["getOnboardingSnapshot", "completeOnboarding", "openPetsFromOnboarding", "openIntegrationsFromOnboarding"]) assert.match(controlCenterPreloadSource, new RegExp(`${method}:`), `Control Center preload must expose exact onboarding method ${method}.`);
assert.match(controlCenterPreloadSource, /getSettingsSnapshot: \(\) => ipcRenderer\.invoke\("openpets:control-center:get-settings-snapshot"\)/, "Control Center preload must expose the read-only settings snapshot query.");
assert.match(controlCenterPreloadSource, /getReactionMappingSnapshot: \(\) => ipcRenderer\.invoke\("openpets:control-center:get-reaction-mapping-snapshot"\)/, "Control Center preload must expose the read-only reaction mapping snapshot query.");
assert.match(controlCenterPreloadSource, /getPetsSnapshot: \(\) => ipcRenderer\.invoke\("openpets:control-center:get-pets-snapshot"\)/, "Control Center preload must expose exact pets snapshot query.");
assert.match(controlCenterPreloadSource, /getPetCatalogSnapshot: \(\) => ipcRenderer\.invoke\("openpets:control-center:get-pet-catalog-snapshot"\)/, "Control Center preload must expose exact pet catalog snapshot query.");
assert.match(controlCenterPreloadSource, /getPetCatalogPage: \(page\) => ipcRenderer\.invoke\("openpets:control-center:get-pet-catalog-page", page\)/, "Control Center preload must expose exact pet catalog page query.");
assert.match(controlCenterPreloadSource, /getPetCatalogSearch: \(\) => ipcRenderer\.invoke\("openpets:control-center:get-pet-catalog-search"\)/, "Control Center preload must expose exact pet catalog search query.");
assert.match(controlCenterPreloadSource, /getCodexPetsSnapshot: \(\) => ipcRenderer\.invoke\("openpets:control-center:get-codex-pets-snapshot"\)/, "Control Center preload must expose exact Codex pets query.");
for (const method of ["getPluginsSnapshot", "setPluginEnabled", "savePluginConfig", "reloadPlugin", "executePluginCommand", "loadLocalPlugin", "getPluginCatalogSnapshot", "installCatalogPlugin", "updateCatalogPlugin", "uninstallPlugin"]) assert.match(controlCenterPreloadSource, new RegExp(`${method}:`), `Control Center preload must expose exact plugin method ${method}.`);
assert.match(controlCenterPreloadSource, /setLaunchAtLogin: \(enabled\) => ipcRenderer\.invoke\("openpets:control-center:set-launch-at-login", enabled\)/, "Control Center preload must expose only the narrow launch-at-login mutation.");
assert.match(controlCenterPreloadSource, /setPetScale: \(value\) => ipcRenderer\.invoke\("openpets:control-center:set-pet-scale", value\)/, "Control Center preload must expose the narrow pet scale mutation.");
assert.match(controlCenterPreloadSource, /resetDefaultPetPosition: \(\) => ipcRenderer\.invoke\("openpets:control-center:reset-default-pet-position"\)/, "Control Center preload must expose the narrow no-arg default pet position reset.");
assert.match(controlCenterPreloadSource, /setReactionAnimationOverride: \(reactionId, animationId\) => ipcRenderer\.invoke\("openpets:control-center:set-reaction-animation-override", reactionId, animationId\)/, "Control Center preload must expose the exact reaction mapping set mutation.");
assert.match(controlCenterPreloadSource, /resetReactionAnimationOverride: \(reactionId\) => ipcRenderer\.invoke\("openpets:control-center:reset-reaction-animation-override", reactionId\)/, "Control Center preload must expose the exact per-reaction reset mutation.");
assert.match(controlCenterPreloadSource, /resetReactionAnimationOverrides: \(\) => ipcRenderer\.invoke\("openpets:control-center:reset-reaction-animation-overrides"\)/, "Control Center preload must expose the exact reset-all reaction mapping mutation.");
assert.match(controlCenterPreloadSource, /checkForUpdates: \(\) => ipcRenderer\.invoke\("openpets:control-center:check-for-updates"\)/, "Control Center preload must expose the exact update check mutation.");
assert.match(controlCenterPreloadSource, /openUpdateReleasePage: \(\) => ipcRenderer\.invoke\("openpets:control-center:open-update-release-page"\)/, "Control Center preload must expose the exact no-arg release page opener.");
for (const [method, channel] of [["setDefaultPet", "set-default-pet"], ["installCatalogPet", "install-catalog-pet"], ["importCodexPet", "import-codex-pet"], ["removeInstalledPet", "remove-installed-pet"]] as const) assert.match(controlCenterPreloadSource, new RegExp(`${method}: \\(petId\\) => ipcRenderer\\.invoke\\("openpets:control-center:${channel}", petId\\)`), `Control Center preload must expose exact ${method} pet mutation.`);
assert.doesNotMatch(controlCenterPreloadSource, /subscribe|send\(|on\(|handle|openExternal|require\("node:|\bfs\b|\bpath\b|\bprocess\b/, "Control Center preload must not expose generic IPC, subscriptions, external-open, Node APIs, or renderer DOM logic.");
assert.match(controlCenterWindowSource, /sandbox:\s*true/, "Control Center window must use sandboxed renderer.");
assert.match(controlCenterWindowSource, /contextIsolation:\s*true/, "Control Center window must use context isolation.");
assert.match(controlCenterWindowSource, /nodeIntegration:\s*false/, "Control Center window must disable Node integration.");
assert.match(controlCenterWindowSource, /setWindowOpenHandler\(\(\) => \(\{ action: "deny" \}\)\)/, "Control Center window must deny window-open.");
assert.match(controlCenterWindowSource, /will-navigate[\s\S]*preventDefault/, "Control Center window must deny navigation.");
assert.match(controlCenterWindowSource, /will-redirect[\s\S]*preventDefault/, "Control Center window must deny redirects.");
assert.match(controlCenterWindowSource, /OPENPETS_CONTROL_CENTER_DEV_URL/, "Control Center dev URL must be behind an explicit env var.");
assert.match(controlCenterWindowSource, /if \(app\.isPackaged\) return null/, "Control Center dev URL must not be honored in packaged builds.");
assert.match(controlCenterWindowSource, /isLoopbackControlCenterDevUrl/, "Control Center dev URL must be validated before loadURL.");
...
assert.match(controlCenterWindowSource, /loadFile\(join\(distDir, "renderer", "control-center", "index\.html"\), route \? \{ hash: route \} : undefined\)/, "Control Center production load must use built renderer loadFile output with optional narrow route hash.");
assert.match(controlCenterIpcSource, /isControlCenterWebContents\(event\.sender\)/, "Control Center IPC must assert the sender webContents.");
assert.match(controlCenterIpcSource, /openpets:control-center:get-settings-snapshot/, "Control Center IPC must expose a sender-guarded read-only settings snapshot.");
assert.match(controlCenterIpcSource, /openpets:control-center:get-reaction-mapping-snapshot/, "Control Center IPC must expose a sender-guarded read-only reaction mapping snapshot.");
for (const channel of ["get-pets-snapshot", "get-pet-catalog-snapshot", "get-pet-catalog-page", "get-pet-catalog-search", "get-codex-pets-snapshot"]) assert.match(controlCenterIpcSource, new RegExp(`openpets:control-center:${channel}`), `Control Center IPC must expose exact ${channel} channel.`);
for (const channel of ["get-plugins-snapshot", "set-plugin-enabled", "save-plugin-config", "reload-plugin", "execute-plugin-command", "load-local-plugin", "get-plugin-catalog-snapshot", "install-catalog-plugin", "update-catalog-plugin", "uninstall-plugin"]) assert.match(controlCenterIpcSource, new RegExp(`openpets:control-center:${channel}`), `Control Center IPC must expose exact plugin ${channel} channel.`);
assert.match(traySource, /Plugins\.\.\.[\s\S]*openControlCenterPluginsWindow\(\)/, "tray Plugins must open narrow Control Center Plugins route.");
assert.doesNotMatch(controlCenterPreloadSource + controlCenterIpcSource, /plugin-(?:invoke|send|subscribe|mutate)|plugins-mutate|control-center:plugin$/, "Control Center plugins must not use a generic plugin bridge.");
assert.match(controlCenterIpcSource, /openpets:control-center:set-launch-at-login/, "Control Center IPC must expose the narrow launch-at-login mutation.");
assert.match(controlCenterIpcSource, /openpets:control-center:set-pet-scale/, "Control Center IPC must expose the narrow pet scale mutation.");
assert.match(controlCenterIpcSource, /openpets:control-center:reset-default-pet-position/, "Control Center IPC must expose the narrow default pet position reset mutation.");
assert.match(controlCenterIpcSource, /openpets:control-center:set-reaction-animation-override/, "Control Center IPC must expose only the exact reaction mapping set channel.");
assert.match(controlCenterIpcSource, /openpets:control-center:reset-reaction-animation-override/, "Control Center IPC must expose only the exact per-reaction reset channel.");
assert.match(controlCenterIpcSource, /openpets:control-center:reset-reaction-animation-overrides/, "Control Center IPC must expose only the exact reset-all reaction mapping channel.");
assert.match(controlCenterIpcSource, /openpets:control-center:check-for-updates/, "Control Center IPC must expose only the exact update check channel.");
assert.match(controlCenterIpcSource, /openpets:control-center:open-update-release-page/, "Control Center IPC must expose only the exact release page open channel.");
for (const channel of ["set-default-pet", "install-catalog-pet", "import-codex-pet", "remove-installed-pet"]) assert.match(controlCenterIpcSource, new RegExp(`openpets:control-center:${channel}`), `Control Center IPC must expose exact ${channel} channel.`);
assert.match(controlCenterIpcSource, /handleControlCenterSetLaunchAtLoginRequest\(event\.sender, getAuthorizedControlCenterSender\(event\), enabled/, "Launch-at-login mutation must authorize sender before validation/write.");
assert.match(controlCenterIpcSource, /handleControlCenterSetPetScaleRequest\(event\.sender, getAuthorizedControlCenterSender\(event\), value/, "Pet scale mutation must authorize sender before validation/write.");
assert.match(controlCenterIpcSource, /handleControlCenterResetDefaultPetPositionRequest\(event\.sender, getAuthorizedControlCenterSender\(event\)/, "Default pet position reset must authorize sender before reset.");
assert.match(controlCenterIpcCoreSource, /assertControlCenterBooleanInput\(enabled, "launchAtLogin\.enabled"\)/, "Launch-at-login mutation must strictly validate boolean input.");
assert.match(controlCenterIpcCoreSource, /assertControlCenterPetScaleInput\(value, deps\.petScaleOptions\)/, "Pet scale mutation must strictly validate against pet scale options.");
assert.match(controlCenterIpcCoreSource, /updatePreferences\(\{ petScale: value \}\)[\s\S]*refreshDefaultPetContent\(\);[\s\S]*refreshAgentPetContent\(\);/, "Pet scale mutation must preserve legacy preference write and pet content refreshes.");
assert.match(controlCenterIpcCoreSource, /setLoginItemSettings\(\{ openAtLogin: enabled, openAsHidden: true \}\)/, "Launch-at-login mutation must preserve legacy login item behavior.");
assert.match(controlCenterIpcCoreSource, /resetDefaultPetToInitialPosition\(\);[\s\S]*return \{ settings: deps\.getSettingsSnapshot\(\) \}/, "Default pet position reset must preserve legacy reset behavior and return refreshed settings.");
assert.match(controlCenterIpcCoreSource, /buildControlCenterReactionMappingSnapshot/, "Reaction mapping snapshot must be built through the testable core helper.");
assert.match(controlCenterIpcCoreSource, /spriteMetadata:[\s\S]*previewUrl:[\s\S]*version:[\s\S]*spriteFileName/, "Reaction mapping snapshot must include scoped preview URL, version, and sprite metadata.");
assert.doesNotMatch(controlCenterIpcCoreSource, /previewSource/, "Reaction mapping snapshot must not expose arbitrary preview source data.");
assert.match(controlCenterIpcCoreSource, /assertAuthorizedControlCenterSender\(sender, controlCenterSender\);[\s\S]*assertControlCenterCatalogPageInput\(page\);[\s\S]*getPage\(page\)/, "Catalog page reads must authorize and validate page before dependency calls.");
assert.match(controlCenterIpcCoreSource, /assertAuthorizedControlCenterSender\(sender, controlCenterSender\);[\s\S]*assertControlCenterSafePetId\(petId,[\s\S]*mutateControlCenterPet\(petId, deps\)/, "Pet writes must authorize and validate pet id before service calls.");
assert.match(controlCenterIpcCoreSource, /writePet\(petId\)[\s\S]*refreshTrayMenu\(\)[\s\S]*buildControlCenterPetsSnapshot/, "Pet writes must refresh tray after successful service write and return sanitized pets snapshot.");
assert.match(controlCenterRendererHtml, /img-src data: https:\/\/openpets\.dev openpets-pet-preview: openpets-codex:/, "Control Center Pets image CSP must allow only data, openpets.dev, and scoped internal protocols.");
assert.doesNotMatch(controlCenterRendererHtml, /img-src[^\"]*'self'/, "Control Center image CSP must not allow generic self-hosted images.");
assert.doesNotMatch(controlCenterRendererHtml, /img-src[^\"]*(?:file:|http:|blob:)/, "Control Center image CSP must not allow file, broad http, or blob images.");
...
assert.match(controlCenterIpcSource, /getLaunchAtLoginState\(\)/, "Control Center settings snapshot must include launch-at-login state.");
assert.match(controlCenterIpcSource, /reactionAnimationMetadata/, "Control Center settings snapshot must include reaction animation summary metadata.");
assert.doesNotMatch(controlCenterIpcSource, /update-reaction|update-preferences|openExternal|openpets:control-center:invoke|openpets:control-center:update(?!(?:-release|-agent-setup-command-paths|-catalog-plugin))|openpets:control-center:send|openpets:control-center:subscribe/, "Control Center IPC must not add broad settings, update, invoke, send, subscribe, or external-open bridges.");
for (const source of [controlCenterPreloadSource, controlCenterIpcSource, controlCenterContractSource]) assert.doesNotMatch(source, /openpets:control-center:(?:pet|catalog|install|remove|update)$|openpets:control-center:(?:install|remove|import|set-default)$|setDefaultPetBridge/, "Control Center must not expose generic pet/catalog/install/remove bridges.");
...
assert.match(controlCenterIpcSource, /recoverDefaultPetMouseInterop/, "Control Center default-pet changes must recover mouse interop for dragging without app restart.");
...
assert.doesNotMatch(traySource, /openTaskWindow\("pet-manager"\)/, "tray pet entries must no longer open the legacy Pet Manager task window.");
assert.match(traySource, /openControlCenterPetsWindow\(\)/, "tray pet entries must open the Control Center Pets route.");
assert.match(controlCenterWindowSource, /openControlCenterPetsWindow[\s\S]*openControlCenterWindowRoute\("pets"\)/, "Control Center must expose a narrow Pets route launcher.");
assert.match(controlCenterWindowSource, /openControlCenterSettingsWindow[\s\S]*openControlCenterWindowRoute\("settings"\)/, "Control Center must expose a narrow Settings route launcher.");
assert.match(controlCenterWindowSource, /openControlCenterOnboardingWindow[\s\S]*openControlCenterWindowRoute\("onboarding"\)/, "Control Center must expose a narrow Onboarding route launcher.");
for (const channel of ["get-onboarding-snapshot", "complete-onboarding", "open-pets-from-onboarding", "open-integrations-from-onboarding"]) assert.match(controlCenterIpcSource, new RegExp(`openpets:control-center:${channel}`), `Control Center IPC must expose exact onboarding channel ${channel}.`);
...
assert.match(controlCenterRendererSource, /Desktop OpenCode setup is global|OpenCode/, "Control Center Integrations must retain OpenCode setup UI.");
assert.match(controlCenterRendererSource, /Cursor/, "Control Center Integrations must retain Cursor setup UI.");
